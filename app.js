...
yargs.command('Request', 'get data of city', {
    ...
    const url = 'https://api.openweathermap.org/data/2.5/weather?q=' + cityNameReplaced + '&appid=8d6e5706b17fb3303800fea3091696e1'
    request({url: url, json: true}, (error, data) => {
        const weatherString = JSON.stringify(data.body.weather)
        const weatherStringBracket_1stRemove = weatherString.replace('[', '')
        const weatherStringBracket_2ndRemove = weatherStringBracket_1stRemove.replace(']', '')
        const weatherJSON = JSON.parse(weatherStringBracket_2ndRemove)
